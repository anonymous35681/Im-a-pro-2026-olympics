...
def run() -> None:
    ...
    df_survey["age_group"] = df_survey["5. Ваш возраст (количество полных лет):"].apply(
        map_age_to_group
    )
    survey_pct = (
        df_survey[df_survey["age_group"] != "Другое"]["age_group"].value_counts(
            normalize=True
        )
        * 100
    ...
    demo_pct = (
        df_demo[df_demo["age_group"] != "Другое"]
        .groupby("age_group")["total_both"]
        .sum()
        / df_demo[df_demo["age_group"] != "Другое"]["total_both"].sum()
        * 100
    ).reindex(age_groups, fill_value=0)
    ...
    for values, color, label in [
        (survey_values, NEON_CYAN, "Выборка (n=1000)"),
        (demo_values, "#FF0055", "Демография региона"),
    ]:
    ...
    plt.yticks(
        [5, 10, 15, 20, 25], ["5%", "10%", "15%", "20%", "25%"], color="grey", size=8
    )
    plt.ylim(0, 30)

    plt.title(
        "Сравнение возрастной структуры:\nВыборка vs Демография региона",
        fontsize=14,
        pad=20,
        color=TEXT_COLOR,
    )
